...
const fmtBytes = gb => `${Number(gb||0).toFixed(1)} GB`;
  ...
  return (
    <div style={{ padding:"10px 12px", background:"var(--surface2)", borderRadius:8, border:"1px solid var(--border)" }}>
      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
        ...
        <span style={{ fontSize:12, color:"var(--muted)" }}>{fmtBytes(disk.freeGB)} livre de {fmtBytes(disk.sizeGB)}</span>
      </div>
...
export default function AgentModal({ equipment, onClose }) {
  ...
  const revokeToken = async (token) => {
    if (!window.confirm("Revogar este token? O agente parará de funcionar nesta máquina.")) return;
  ...
  if (latest && !isOnline) {
    alerts.push({ level: "warn", msg: `Sem checkin há ${relativeTime(latest.collected_at)}` });
  }
  if (data.antivirus && data.antivirus.length === 0) {
    alerts.push({ level: "warn", msg: "Nenhum antivírus detectado" });
  }
        ...
        {loading ? (
          ...
          <Card style={{ textAlign:"center", padding:40 }}>
            ...
            <div style={{ fontSize:13, color:"var(--muted)", marginBottom:18 }}>
              Gere um token na aba "🔑 Tokens" e instale o TechOS Agent no PC do cliente.
            </div>
              ...
              <>
                <div style={{ display:"flex", gap:10, marginBottom:14, alignItems:"center", justifyContent:"space-between", flexWrap:"wrap" }}>
                  <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                    ...
                    <span style={{ fontSize:12, color:"var(--muted)" }}>
                      Último checkin: {relativeTime(latest.collected_at)} ({fmtDate(latest.collected_at)})
                    </span>
                  </div>
                  <div style={{ fontSize:11, color:"var(--muted)" }}>
                    Agente v{data.agentVersion || "?"}
                  </div>
                </div>
                ...
                {alerts.length > 0 && (
                  <Card style={{ marginBottom:12, border:"1px solid rgba(245,197,66,.4)", background:"rgba(245,197,66,.06)" }}>
                    ...
                    {alerts.map((a,i) => (
                      <div key={i} style={{ fontSize:12, color: a.level === "critical" ? "var(--red)" : "var(--muted)", marginTop:3 }}>
                        • {a.msg}
                      </div>
                    ))}
                  </Card>
                ...
                <div style={{ display:"flex", gap:8, marginBottom:10, flexWrap:"wrap" }}>
                  ...
                  <StatCard label="IP Local" value={data.network?.[0]?.ip || "—"} subtitle={data.network?.[0]?.interface || ""} />
                </div>

                {/* Discos */}
                {data.logicalDisks && data.logicalDisks.length > 0 && (
                  <div style={{ marginTop:14 }}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                      {data.logicalDisks.map((d,i) => <DiskBar key={i} disk={d} />)}
                    </div>
                  </div>
                )}
              </>
              ...
              <>
                  ...
                  <Card>
                    ...
                    {(data.gpus || []).map((g,i) => (
                      <div key={i} style={{ marginBottom:6 }}>
                        ...
                        <div style={{ fontSize:11, color:"var(--muted)" }}>Driver: {g.driver}</div>
                      </div>
                    ))}
                  </Card>
                </div>
                ...
                <Card style={{ marginTop:10 }}>
                  <div style={{ fontSize:11, fontWeight:700, color:"var(--accent)", textTransform:"uppercase", marginBottom:8 }}>
                    💾 Memória RAM — Total: {data.ram?.totalGB} GB
                  </div>
                  ...
                  {(data.physicalDisks || []).map((d,i) => (
                    <div key={i} style={{ padding:"8px 10px", background:"var(--surface2)", borderRadius:6, marginBottom:6 }}>
                      <div style={{ display:"flex", justifyContent:"space-between" }}>
                        ...
                        <span style={{ fontSize:11, color: d.healthStatus === "Healthy" ? "var(--green)" : "var(--red)", fontWeight:600 }}>
                          {d.healthStatus === "Healthy" ? "✓ Saudável" : `⚠ ${d.healthStatus}`}
                        </span>
                      </div>
                      <div style={{ fontSize:11, color:"var(--muted)", marginTop:3 }}>
                        {d.mediaType} · {d.sizeGB} GB · SN: {d.serial}
                      </div>
                    </div>
                  ))}
                </Card>
                  ...
                  {(data.network || []).map((n,i) => (
                    <div key={i} style={{ padding:"8px 10px", background:"var(--surface2)", borderRadius:6, marginBottom:6, fontSize:12 }}>
                      ...
                      <div style={{ color:"var(--muted)", marginTop:3 }}>
                        IP: {n.ip} · MAC: {n.mac} · Gateway: {n.gateway || "—"}
                      </div>
                    </div>
                  ))}
                </Card>
              ...
              <>
                <Card style={{ marginBottom:10 }}>
                  ...
                  {(data.antivirus || []).length === 0 ? (
                    <div style={{ color:"var(--red)", fontSize:13 }}>⚠ Nenhum antivírus detectado</div>
                  ) : (
                    (data.antivirus || []).map((av,i) => (
                      <div key={i} style={{ fontSize:13, padding:"6px 0" }}>✓ {av.name}</div>
                    ))
                  )}
                </Card>
                ...
                {data.office && (
                  <Card style={{ marginBottom:10 }}>
                    <div style={{ fontSize:11, fontWeight:700, color:"var(--accent)", textTransform:"uppercase", marginBottom:8 }}>📊 Office</div>
                    <div style={{ fontSize:13 }}>{data.office}</div>
                  </Card>
                )}

                <Card>
                  <div style={{ fontSize:11, fontWeight:700, color:"var(--accent)", textTransform:"uppercase", marginBottom:8 }}>
                    📦 Programas Instalados ({data.software?.length || 0})
                  </div>
              ...
              <>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                  <div style={{ fontSize:13, color:"var(--muted)" }}>
                    Tokens vinculam o agente neste PC ao equipamento. Cada PC precisa do seu próprio token.
                  </div>
                ...
                {tokens.length === 0 ? (
                  <Card style={{ textAlign:"center", padding:30, color:"var(--muted)" }}>
                    Nenhum token gerado. Clique em "+ Gerar Token" para começar.
                  </Card>
                ) : (
                  <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    {tokens.map(t => (
                        ...
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10 }}>
                          <div style={{ flex:1, minWidth:0 }}>
                            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
                              ...
                              {t.last_checkin && (Date.now() - new Date(t.last_checkin).getTime()) < 86400000 ? (
                                <StatusBadge status="Online" color="online" icon="●" />
                              ) : t.last_checkin ? (
                                <StatusBadge status="Offline" color="offline" icon="○" />
                              ) : (
                                <StatusBadge status="Aguardando" color="warn" icon="⏳" />
                              )}
                            </div>
                            <div style={{ fontFamily:"monospace", fontSize:11, color:"var(--muted)", padding:"4px 8px", background:"var(--surface2)", borderRadius:4, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                              {t.token}
                            </div>
                            <div style={{ fontSize:11, color:"var(--muted)", marginTop:4 }}>
                              Criado: {fmtDate(t.created_at)} · Último checkin: {t.last_checkin ? relativeTime(t.last_checkin) : "nunca"}
                            </div>
                          </div>
                          <div style={{ display:"flex", gap:4 }}>
                            <Btn small variant={copied === t.token ? "success" : "secondary"} onClick={() => copy(t.token, t.token)}>
                ...
                <Card style={{ marginTop:14, background:"rgba(79,142,247,.08)", border:"1px solid rgba(79,142,247,.25)" }}>
                  ...
                  <ol style={{ fontSize:12, color:"var(--muted)", paddingLeft:20, lineHeight:1.6 }}>
                    ...
                    <li>No PC do cliente, abra PowerShell <strong>como Administrador</strong></li>
                    <li>Execute: <code style={{ background:"var(--surface2)", padding:"2px 6px", borderRadius:4 }}>.\installer.ps1 -Token "SEU_TOKEN"</code></li>
                    <li>Aguarde a primeira coleta (até 1 minuto)</li>
                  </ol>
                </Card>
              </>
            )}
              ...
              <>
                {configLoading ? (
                  ...
                  <Card style={{ textAlign:"center", padding:32, color:"var(--muted)" }}>
                    ...
                    <div style={{ fontSize:12, marginTop:6 }}>Aprove um token primeiro.</div>
                  </Card>
                ) : (
                    ...
                    <Card>
                      ...
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                        {[
                          ...
                          { label:"2x/dia", value:12 },
                          { label:"4x/dia", value:6 },
                          { label:"A cada hora", value:1 },
                        ].map(opt => (
                    ...
                    <Card>
                      ...
                      {[
                        ...
                        <label key={item.key} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 0", borderBottom:"1px solid var(--border)", cursor:"pointer" }}>
                          <div style={{
                            width:40, height:22, borderRadius:99, position:"relative", transition:"all .2s", flexShrink:0,
                            background: config[item.key] ? "var(--accent)" : "var(--border)",
                          }} onClick={() => setConfig(c => ({...c, [item.key]: !c[item.key]}))}>
                            <div style={{
                              width:18, height:18, borderRadius:99, background:"#fff", position:"absolute",
                              top:2, left: config[item.key] ? 20 : 2, transition:"all .2s",
                            }} />
                          </div>
                          <div>
                      ...
                      <div style={{ marginBottom:12 }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                          <input type="range" min={5} max={30} value={config.alert_disk_pct || 10}
                            onChange={e => setConfig(c => ({...c, alert_disk_pct: parseInt(e.target.value)}))}
                            style={{ flex:1 }} />
                          <span style={{ fontWeight:700, color:"var(--yellow)", minWidth:40 }}>{config.alert_disk_pct || 10}%</span>
                        </div>
                        <div style={{ fontSize:11, color:"var(--muted)" }}>Alerta quando espaço livre ficar abaixo de {config.alert_disk_pct || 10}%</div>
                      </div>
                      <div>
                        <div style={{ fontSize:12, fontWeight:600, marginBottom:6 }}>Serviços monitorados</div>
                        <div style={{ fontSize:11, color:"var(--muted)", marginBottom:8 }}>Um por linha. Ex: WinDefend, MSSQLSERVER, W3SVC</div>
                        <textarea
...
').map(s => s.trim()).filter(Boolean)}))}
                      ...
                      <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                        ...
                        <tbody>
                          {usageSummary.map((d, i) => {
                            ...
                            const activeColor = activeH >= 6 ? "var(--green)" : activeH >= 3 ? "var(--yellow)" : "var(--red)";
                            return (
                              <tr key={i} style={{ borderBottom:"1px solid var(--border)" }}>
                                ...
                                <td style={{ padding:"8px 12px", fontWeight:700, color:activeColor }}>{activeH}h</td>
            ...
            {tab === "history" && (
              <>
                {snapshots.length === 0 ? (
                  <Card style={{ textAlign:"center", padding:30, color:"var(--muted)" }}>
                    Nenhum snapshot recebido ainda.
                  </Card>
                ) : (
                  <Card style={{ padding:0 }}>
                    <table style={{ width:"100%", fontSize:12 }}>
                      ...
                      <tbody>
                        {snapshots.map(s => (
                          <tr key={s.id} style={{ borderBottom:"1px solid var(--border)" }}>
                            ...
                            <td style={{ padding:"8px 12px", color:"var(--muted)" }}>{s.data?.network?.[0]?.ip || "—"}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
